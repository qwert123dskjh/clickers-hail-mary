...
document.addEventListener("DOMContentLoaded", () => {
    ...
    setInterval(() => {vladis += cursors;saveGame();updateDisplay();}, 1000);// Active button affordable style updating loop
    setInterval(() => {if (cursorButton) {if (vladis >= cursorPrice) {cursorButton.classList.add("affordable");} else {cursorButton.classList.remove("affordable");}}if (clickPowerButton) {if (vladis >= clickPowerPrice) {clickPowerButton.classList.add("affordable");} else {clickPowerButton.classList.remove("affordable");}}}, 100);});
